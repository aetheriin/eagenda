<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Master Klasifikasi Naskah Dinas') }}
        </h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <h3 class="text-lg font-bold text-gray-800">Daftar Klasifikasi Naskah Dinas</h3>

                <div class="flex items-center gap-4">
                    <!-- Dropdown Filter -->
                    <form method="GET" action="{{ route('klasifikasi-naskah.index') }}">
                    ...
                    <form method="GET" action="{{ route('klasifikasi-naskah.index') }}" class="flex items-center space-x-2">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border border-gray-300 text-sm">
                        ...
                        <tbody>
                            @forelse($klasifikasiNaskah as $index => $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 border text-center">
                                        {{ $loop->iteration + ($klasifikasiNaskah->currentPage() - 1) * $klasifikasiNaskah->perPage() }}
                                    </td>
                                    <td class="px-4 py-2 border">{{ $item->kode_klasifikasi }}</td>
                                    <td class="px-4 py-2 border">{{ $item->nama_klasifikasi }}</td>
                                </tr>
                            @empty
                                <tr>
