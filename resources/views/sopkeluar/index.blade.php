                    ...
                    <table class="w-full border border-gray-300 text-sm">
                        <thead class="bg-gray-100 text-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">Nama SOP/File</th>
                                <th class="px-4 py-2 border">Tim/Subtim</th>
                                <th class="px-4 py-2 border">Tanggal Dibuat</th>
                                <th class="px-4 py-2 border">Tanggal Berlaku</th>
                                <th class="px-4 py-2 border text-center">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($sopKeluar as $index => $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 border text-center">
                                        {{ $loop->iteration + ($sopKeluar->currentPage() - 1) * $sopKeluar->perPage() }}
                                    </td>
                                    <td class="px-4 py-2 border">{{ $item->nomor_naskah }}</td>
                                    <td class="px-4 py-2 border">{{ $item->nama_sop }}</td>
                                    <td class="px-4 py-2 border">{{ $item->subtim->nama_subtim ?? '-' }}</td>
                                    <td class="px-4 py-2 border text-center">
