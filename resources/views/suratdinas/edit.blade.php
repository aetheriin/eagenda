    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-6">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form action="{{ route('surat-dinas.update', $suratDinas->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <!-- Nomor Naskah -->
                    <x-input-nomor-naskah :value="$suratDinas->nomor_naskah" readonly="true" />

                    <!-- Keamanan Surat -->
                    <div class="mb-4">
                        <label class="block text-gray-700 font-semibold mb-1" for="keamanan_surat_id">Keamanan Surat <span class="text-red-500">*</span></label>
                        <select name="keamanan_surat_id" id="keamanan_surat_id"
                            ...
                            @foreach($keamananSurat as $item)
                                <option value="{{ $item->id }}" {{ old('keamanan_surat_id', $suratDinas->keamanan_surat_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex gap-4 mb-4">
                        ...
                        <div class="w-3/4">
                            ...
                            <select name="bagian_fungsi_id" id="bagian_fungsi_id"
                                ...
                                @foreach($bagianFungsi as $bagian)
                                    <option value="{{ $bagian->id }}" {{ old('bagian_fungsi_id', $suratDinas->bagian_fungsi_id) == $bagian->id ? 'selected' : '' }}>
                                        {{ $bagian->nama_bagian }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="w-1/3">
                            ...
                            <input type="text" name="klasifikasi_naskah_id" 
                                value="{{ old('klasifikasi_naskah_id', $suratDinas->klasifikasiNaskah->nama_klasifikasi ?? '') }}"
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" required />
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="perihal" value="{{ old('perihal', $suratDinas->perihal) }}" required
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="tujuan_penerima" value="{{ old('tujuan_penerima', $suratDinas->tujuan_penerima) }}" required
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal" value="{{ old('tanggal', $suratDinas->tanggal) }}" required
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if($suratDinas->file)
                            <p class="text-sm text-gray-500 mt-1">File saat ini:
                                <a href="{{ asset('storage/' . $suratDinas->file) }}" target="_blank" class="text-blue-600 underline">Lihat File</a>
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="keterangan" rows="3"
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('keterangan', $suratDinas->keterangan) }}</textarea>
                    </div>

                    <!-- Tombol -->
                    <div class="flex justify-end gap-4 mt-4">
                        <a href="{{ route('surat-dinas.index') }}" 
