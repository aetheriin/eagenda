    ...
    <div class="py-8">
        ...
        @endif
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <h3 class="text-lg font-bold text-gray-800">Daftar Surat Tugas Keluar</h3>

                <div class="flex items-center gap-4">
                    <!-- Dropdown Filter -->
                    <form method="GET" action="{{ route('surat-tugas.index') }}">
                    ...
                    <form method="GET" action="{{ route('surat-tugas.index') }}" class="flex items-center space-x-2">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border border-gray-300 text-sm">
                        ...
                        <tbody>
                            @forelse($suratTugas as $index => $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 border text-center">
                                        {{ $loop->iteration + ($suratTugas->currentPage() - 1) * $suratTugas->perPage() }}
                                    </td>
                                    ...
                                    <td class="px-4 py-2 border text-center">
                                        <div class="flex justify-center items-center gap-3">
                                            <!-- Tombol Edit -->
                                            <a href="{{ route('surat-tugas.edit', $item->id) }}" 
                                            ...
                                            <!-- Tombol Hapus -->
                                            <form action="{{ route('surat-tugas.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                    class="inline-flex items-center text-white px-3 py-2 rounded text-sm font-semibold"
