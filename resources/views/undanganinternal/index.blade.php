    ...
    <div class="py-8">
        ...
        @endif
            ...
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <h3 class="text-lg font-bold text-gray-800">Daftar Undangan Internal Keluar</h3>

                <div class="flex items-center gap-4">
                    <!-- Dropdown Filter -->
                    <form method="GET" action="{{ route('undangan-internal.index') }}">
                    ...
                    <a href="{{ route('undangan-internal.create') }}" 
                ...
                <div class="overflow-x-auto">
                    <table class="w-full border border-gray-300 text-sm">
                        ...
                        <tbody>
                            @forelse($undanganInternal as $index => $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 border text-center">
                                        {{ $loop->iteration + ($undanganInternal->currentPage() - 1) * $undanganInternal->perPage() }}
                                    </td>
                                    ...
                                    <td class="px-4 py-2 border text-center">
                                        <div class="flex justify-center items-center gap-3">
                                            <!-- Tombol Edit -->
                                            <a href="{{ route('undangan-internal.edit', $item->id) }}" 
                                            ...
                                            <!-- Tombol Hapus -->
                                            <form action="{{ route('undangan-internal.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                    class="inline-flex items-center text-white px-3 py-2 rounded text-sm font-semibold"
