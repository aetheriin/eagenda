        ...
        <div class="max-w-4xl mx-auto px-6">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <h2 class="text-l font-semibold text-gray-800 mb-6">
                    Edit Undangan Eksternal Keluar
                </h2>
                ...
                <form action="{{ route('undangan-eksternal.update', $undanganEksternal->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <!-- Nomor Naskah -->
                    <x-input-nomor-naskah :value="$undanganEksternal->nomor_naskah" readonly="true" />
                        ...
                        <select name="keamanan_surat_id" id="keamanan_surat_id"
                            ...
                            @foreach($keamananSurat as $keamanan)
                                <option value="{{ $keamanan->id }}"
                                    {{ old('keamanan_surat_id', $undanganEksternal->keamanan_surat_id) == $keamanan->id ? 'selected' : '' }}>
                                    {{ $keamanan->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex gap-4 mb-4">
                            ...
                            <select name="bagian_fungsi_id" id="bagian_fungsi_id"
                                ...
                                @foreach($bagianFungsi as $bagian)
                                    <option value="{{ $bagian->id }}"
                                        {{ old('bagian_fungsi_id', $undanganEksternal->bagian_fungsi_id) == $bagian->id ? 'selected' : '' }}>
                                        {{ $bagian->nama_bagian }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="w-1/3">
                            ...
                            <input type="text" name="klasifikasi_naskah_id"
                                value="{{ old('klasifikasi_naskah_id', $undanganEksternal->klasifikasiNaskah->nama_klasifikasi ?? '') }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="perihal" value="{{ old('perihal', $undanganEksternal->perihal) }}" required
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="tujuan_penerima" value="{{ old('tujuan_penerima', $undanganEksternal->tujuan_penerima) }}" required
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal" value="{{ old('tanggal', $undanganEksternal->tanggal) }}" required
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if($undanganEksternal->file)
                            <p class="text-sm text-gray-500 mt-1">
                                File saat ini:
                                <a href="{{ asset('storage/' . $undanganEksternal->file) }}" target="_blank" class="text-blue-600 underline">Lihat File</a>
                            </p>
                        @endif
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="keterangan" rows="3"
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('keterangan', $undanganEksternal->keterangan) }}</textarea>
                    </div>

                    <!-- Tombol -->
                    <div class="flex justify-end gap-4 mt-4">
                        <a href="{{ route('undangan-eksternal.index') }}" 
