<?php

namespace App\Http\Controllers\Master;

use App\Http\Controllers\Controller;
use App\Models\UndanganEksternal;
use App\Models\KeamananSurat;
use App\Models\BagianFungsi;
use App\Models\KlasifikasiNaskah; 
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;

class UndanganEksternalController extends Controller
{
    public function index(Request $request)
    {
        $query = undanganEksternal::query();

        if ($request->has('search')) {
            $search = $request->input('search');
            $query->where('nomor_naskah', 'like', "%{$search}%")
                ->orWhere('perihal', 'like', "%{$search}%");
        }

        $perPage = $request->input('per_page', 10);
        $undanganEksternal = $query->orderBy('id', 'desc')->paginate($perPage);

        return view('undanganeksternal.index', compact('undanganEksternal'));
    }

    public function create()
    {
        $last = UndanganEksternal::latest('id')->first();
        $nomorUrut = $last ? $last->id + 1 : 1;
        $bagianFungsi = BagianFungsi::all();
        $klasifikasiNaskah = KlasifikasiNaskah::all();
        $keamananSurat = KeamananSurat::all();

        return view('undanganeksternal.create', compact('nomorUrut', 'bagianFungsi', 'keamananSurat', 'klasifikasiNaskah'));
    }

    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                'nomor_urut'                    => 'required',
                'keamanan_surat_id'             => 'required|exists:keamanan_surats,id',
                'bagian_fungsi_id'              => 'required|exists:bagian_fungsis,id',
                'klasifikasi_naskah_id'         => 'required|string',
                'perihal'                       => 'required|string',
                'tujuan_penerima'               => 'required|string',
                'tanggal'                       => 'required|date',
                'file'                          => 'nullable|mimes:pdf,doc,docx|max:2048',
                'keterangan'                    => 'nullable|string',
            ]);

            $bagianFungsi = BagianFungsi::findOrFail($validated['bagian_fungsi_id']);
            $keamananSurat = KeamananSurat::findOrFail($validated['keamanan_surat_id']);
            $klasifikasiNaskah  = KlasifikasiNaskah::where('nama_klasifikasi', 'like', '%' . $request->klasifikasi_naskah_id . '%')
                ->orWhere('kode_klasifikasi', 'like', '%' . $request->klasifikasi_naskah_id . '%')
                ->firstOrFail();

            $nomorUrut = str_pad($validated['nomor_urut'], 2, '0', STR_PAD_LEFT);
            
            $nomorNaskah = 'B-'.$nomorUrut
                . '/' . $bagianFungsi->kode_bps
                . '/' . $klasifikasiNaskah->kode_klasifikasi
                . '/' . now()->year;

            $path = $request->hasFile('file')
                ? $request->file('file')->store('undangan_eksternal', 'public')
                : null;

            UndanganEksternal::create([
                'nomor_naskah'          => $nomorNaskah,
                'keamanan_surat_id'     => $keamananSurat->id,
                'bagian_fungsi_id'      => $bagianFungsi->id,
                'klasifikasi_naskah_id' => $klasifikasiNaskah->id, 
                'perihal'               => $validated['perihal'],
                'tujuan_penerima'       => $validated['tujuan_penerima'],
                'tanggal'               => $validated['tanggal'],
                'file'                  => $path,
                'keterangan'            => $validated['keterangan'] ?? null,
            ]);

            return redirect()->route('undangan-eksternal.index')->with('success', 'Undangan Eksternal berhasil ditambahkan!');
        } catch (\Illuminate\Validation\ValidationException $e) {
            return back()->withErrors($e->errors())->withInput();
        } catch (\Exception $e) {
            return back()->with('error', 'Terjadi kesalahan: ' . $e->getMessage())->withInput();
        }
    }


    public function edit($id)
    {
        $undanganEksternal = UndanganEksternal::findOrFail($id);
        $bagianFungsi = BagianFungsi::all();
        $klasifikasiNaskah = KlasifikasiNaskah::all();
        $keamananSurat = KeamananSurat::all();

        return view('undanganeksternal.edit', compact('undanganEksternal', 'bagianFungsi', 'keamananSurat', 'klasifikasiNaskah'));
    }

    public function update(Request $request, UndanganEksternal $undanganEksternal)
    {
        $request->validate([
            'keamanan_surat_id'         => 'required|exists:keamanan_surats,id',
            'bagian_fungsi_id'          => 'required|exists:bagian_fungsis,id',
            'klasifikasi_naskah_id'     => 'required|string',
            'perihal'                   => 'required|string|max:255',
            'tujuan_penerima'           => 'required|string|max:255',
            'tanggal'                   => 'required|date',
            'file'                      => 'nullable|mimes:pdf,doc,docx|max:2048',
            'keterangan'                => 'nullable|string',
        ]);

        try {
            DB::beginTransaction();

            $bagianFungsi = BagianFungsi::findOrFail($request->bagian_fungsi_id);
            $keamananSurat = KeamananSurat::findOrFail($request->keamanan_surat_id);
            $klasifikasiNaskah  = KlasifikasiNaskah::where('nama_klasifikasi', 'like', '%' . $request->klasifikasi_naskah_id . '%')
                ->orWhere('kode_klasifikasi', 'like', '%' . $request->klasifikasi_naskah_id . '%')
                ->firstOrFail();

            $oldNomorUrut = strtok($undanganEksternal->nomor_naskah, '/');

            $nomorNaskah  = $oldNomorUrut
                . '/' . $bagianFungsi->kode_bps
                . '/' . $klasifikasiNaskah->kode_klasifikasi
                . '/' . now()->year;

            $data = [
                'keamanan_surat_id'     => $keamananSurat->id,
                'bagian_fungsi_id'      => $bagianFungsi->id,
                'klasifikasi_naskah_id' => $klasifikasiNaskah->id, 
                'perihal'               => $request->perihal,
                'tujuan_penerima'       => $request->tujuan_penerima,
                'tanggal'               => $request->tanggal,
                'keterangan'            => $request->keterangan ?? null,
                'nomor_naskah'          => $nomorNaskah,
            ];

            if ($request->hasFile('file')) {
                if ($undanganEksternal->file && Storage::disk('public')->exists($undanganEksternal->file)) {
                    Storage::disk('public')->delete($undanganEksternal->file);
                }
                $data['file'] = $request->file('file')->store('undangan_eksternal', 'public');
            }

            $undanganEksternal->update($data);

            DB::commit();
            return redirect()->route('undangan-eksternal.index')->with('success', 'Undangan Eksternal berhasil diperbarui!');
        } catch (\Exception $e) {
            DB::rollBack();
            return back()->withErrors(['error' => 'Gagal mengupdate data: ' . $e->getMessage()]);
        }
    }

    public function destroy(UndanganEksternal $undanganEksternal)
    {
        try {
            if ($undanganEksternal->file && Storage::disk('public')->exists($undanganEksternal->file)) {
                Storage::disk('public')->delete($undanganEksternal->file);
            }

            $undanganEksternal->delete();

            return redirect()->route('undangan-eksternal.index')->with('success', 'Undangan Eksternal berhasil dihapus!');
        } catch (\Exception $e) {
            return back()->withErrors(['error' => 'Gagal menghapus data: ' . $e->getMessage()]);
        }
    }
}
